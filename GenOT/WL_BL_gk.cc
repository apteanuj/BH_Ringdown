...
int main(int argc, char **argv)
{
  ...
  if (argc != 7) {
    ...
    cerr << "      3. Mino time in trajectory to start WL" << endl;
    cerr << "      4. BL time at which to stop WL" << endl;
    cerr << "      5. Initial value of angular phase chi (degrees)" << endl;
    cerr << "      6. Initial value of axial angle phi (degrees)" << endl;
    exit(0);
  }
  ...
  Real lstart = (Real)atof(argv[3]);
  const Real tstop = (Real)atof(argv[4]);
  const Real chi0 = (Real)atof(argv[5])*DEGTORAD;
  const Real phi0 = (Real)atof(argv[6])*DEGTORAD;
  ...
  while (pass < 2) {
    ...
    // Modified by GK to pick the starting location 
    // automatically for a set inspiral length
    if ( gk_t < 0.0 ) { 
      lstart = lstart+5;
      printf("lstart %lf\n",lstart); 
      pass = 0; 
    ...
    if ( (gk_t < tstop)&&(gk_t > 0.0) ) { 
      lstart = lstart-1;
      printf("lstart %lf, t %lf\n",lstart,gk_t); 
      pass = 0; 
      gk_t = -1.0; 
      } 
    // GK modification completed

    l = lstart;
    phi = phi0;
    chi = chi0;
    for (t = 0; t <= tstop + 500.0 + 500.0*pass; t += dt) { // modified by GK
      //
      ...
      if (pass == 0) { if (rh < 1.1+sqrt(1-a*a)){ if (gk_t < 0.0) gk_t = t;}   }  // modified by GK
      if (pass == 1)
	fprintf(outfile,
