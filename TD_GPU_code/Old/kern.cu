...
__device__ double bb_h[M * N];
__device__ double cc_h[M * N];
__device__ double ee_h[M * N];
__device__ double a_re_h[M * N];
__device__ double a_im_h[M * N];
__device__ double d_re_h[M * N];
__device__ double d_im_h[M * N];
__device__ double v_re_h[M * N];
__device__ double v_im_h[M * N];
...
kernel_init (double dtheta_in, double mass_in, double aa_in, double ss_in, double mm_in,
	     double dx_in, double dt_in, double *theta, double *r_h, double *r_c)
{
  int k;
  int j;
  ...
  if (k >= 1 && k < M - 1)
	 ...
	 CImag = (2.0*em*Om*spin*(Power(eR,2) + 2.0*eR*mass*Om - 3.0*Power(Om,2)*Power(spin,2)))/(bir*eR);
         ...
         bb_h[idx (k, j)] = - (Ataurho + sqrt(Ataurho*Ataurho+4.0*Arhorho))/2.0;
         cc_h[idx (k, j)] = Ayy;
         ee_h[idx (k, j)] = Ataurho + bb_h[idx (k, j)];

         a_re_h[idx (k, j)] = -BtauReal;
         a_im_h[idx (k, j)] = -BtauImag;

         d_re_h[idx (k, j)] = BrhoReal -bb_h[idx (k, j)]*BtauReal; 
         d_im_h[idx (k, j)] = BrhoImag -bb_h[idx (k, j)]*BtauImag;

         v_re_h[idx (k, j)] = -CReal;
         v_im_h[idx (k, j)] = -CImag;
	     ...
	     double *theta, double *r_h)
{
  ...
  double ll_re, ll_im;
         ...
         dbdx = 0.0;
         if (a >= 1 && a < N-1)
         dbdx = (bb_h[idx (b, a+1)]-bb_h[idx (b, a-1)])/dx/2.0;
         dre = d_re_h[idx (b, a)] - dbdx*ee_h[idx (b, a)];
  ...
  if (b >= 1 && b < M - 1)
	...
	rhs_qre[idx (b, a)] = -bb_h[idx (b, a)] * qrex + pre_h[idx (b, a)];
	rhs_qim[idx (b, a)] = -bb_h[idx (b, a)] * qimx + pim_h[idx (b, a)];

	rhs_pre[idx (b, a)] = tred[b][a] +
	  ee_h[idx (b, a)] * prex
	  + cc_h[idx (b, a)] * ll_re
	  + dre * qrex - d_im_h[idx (b, a)] * qimx
	  - a_re_h[idx (b, a)] * pre_h[idx (b, a)] + a_im_h[idx (b, a)] * pim_h[idx (b, a)]
	  - v_re_h[idx (b, a)] * qre_h[idx (b, a)] + v_im_h[idx (b, a)] * qim_h[idx (b, a)];

	rhs_pim[idx (b, a)] = timd[b][a] +
	  ee_h[idx (b, a)] * pimx
	  + cc_h[idx (b, a)] * ll_im
	  + dre * qimx + d_im_h[idx (b, a)] * qrex
	  - a_re_h[idx (b, a)] * pim_h[idx (b, a)] - a_im_h[idx (b, a)] * pre_h[idx (b, a)]
	  - v_re_h[idx (b, a)] * qim_h[idx (b, a)] - v_im_h[idx (b, a)] * qre_h[idx (b, a)];
	     ...
	     double *theta, double *r_c)
{
  ...
  double ll_re, ll_im;
         ...
         dbdx = 0.0;
         if (a >= 1 && a < N-1)
         dbdx = (bb_c[idx (b, a+1)]-bb_c[idx (b, a-1)])/dx/2.0;
  ...
  if (b >= 1 && b < M - 1)
	...
	rhs_qre[idx (b, a)] = -bb_c[idx (b, a)] * qrex + pre_c[idx (b, a)];
	rhs_qim[idx (b, a)] = -bb_c[idx (b, a)] * qimx + pim_c[idx (b, a)];
...
__global__ void
kernel_boundary2b (double *qre, double *qim, double *pre, double *pim, double *d_qre_buff, double *d_qim_buff)
{
  int b, l, i;
